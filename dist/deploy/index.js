...
var Secrets = class {
  constructor(authToken, deploymentLocation) {
    this.authToken = authToken;
    this.location = new URL(import_path.default.posix.join(deploymentLocation.href, "secrets"));
  }
  postSecretsString(secretString, fetch3) {
    return __async(this, null, function* () {
      console.log(`Calling endpoint ${this.location.href} with a secret : ${secretString.length} bytes`);
      const secretAsObject = JSON.parse(secretString);
      const result = yield fetch3(this.location.href, {
        method: "POST",
...
var run = () => __async(exports, null, function* () {
  ...
  if (secrets) {
    ...
    yield secretManager.postSecretsString(secrets, import_node_fetch2.default);
