...
var require_lib2 = __commonJS({
  "node_modules/@octokit/request/node_modules/node-fetch/lib/index.js"(exports2, module2) {
    "use strict";
    Object.defineProperty(exports2, "__esModule", { value: true });
...
var checkStatus = (authToken, location) => __async(void 0, null, function* () {
  const response = yield (0, import_node_fetch.default)(location, {
    method: "GET",
    headers: {
...
var deploy = (authToken, deployment) => __async(void 0, null, function* () {
  ...
  const response = yield (0, import_node_fetch.default)(`${deployment.uri}/${deployment.type}`, {
    method: "POST",
    body: params,
    headers: {
...
var getProbeConfiguration = (core2, probeType) => {
  ...
  const path = core2.getInput(`${probeType}-path`) || void 0;
  const command = core2.getInput(`${probeType}-command`) ? [core2.getInput(`${probeType}-command`)] : void 0;
  if (!path && !command) {
    return void 0;
  }
  return {
...
var getVolumeConfig = (volumesInput, volumeType) => {
  return volumesInput.map((v) => JSON.parse(v)).map((v) => ({
    name: v.name,
    volume: {
      readOnly: v.readOnly,
      volumeType,
...
function getWorkflowFile() {
  ...
  return process.env.GITHUB_WORKFLOW || import_github.context.workflow || "unknown";
}
function getRepo(repoName) {
  var _a, _b;
  const fullOverride = (_a = process.env.TFSO_REPOSITORY) == null ? void 0 : _a.trim();
  if (fullOverride)
    return fullOverride;
  const owner = ((_b = process.env.TFSO_REPOSITORY_OWNER) == null ? void 0 : _b.trim()) || import_github.context.repo.owner;
  return `${owner}/${repoName}`;
}
function getCommitSha() {
  var _a;
  const fromEnv = (_a = process.env.DD_GIT_COMMIT_SHA) == null ? void 0 : _a.trim();
  if (fromEnv)
    return fromEnv;
  return process.env.GITHUB_SHA || import_github.context.sha || "unknown";
}
function getRepoUrl(owner, repo) {
  var _a;
  const fromEnv = (_a = process.env.DD_GIT_REPOSITORY_URL) == null ? void 0 : _a.trim();
  if (fromEnv)
    return fromEnv;
...
var run = () => __async(exports, null, function* () {
  ...
  const httpEndpoint = core.getInput("http-endpoint") || void 0;
  const owner = import_github.context.repo.owner;
  const repository = import_github.context.repo.repo;
  const isSingleton = core.getBooleanInput("singleton");
  const team = core.getInput("team");
  const module2 = core.getInput("module");
  ...
  const envVariables = __spreadProps(__spreadValues({}, getEnvironmentVariables(process.env)), {
    TFSO_REPOSITORY,
    TFSO_WORKFLOW_FILE,
    DD_GIT_COMMIT_SHA,
  ...
  const allowedOrigins = (_a = core.getInput("cors-allowed-origins")) != null ? _a : void 0;
  const volumes = getVolumeConfig(core.getMultilineInput("persistent-volumes"), "persistentVolumeClaim");
  const volumeMounts = core.getMultilineInput("volume-mounts").map((v) => JSON.parse(v));
  ...
  const secrets = __spreadValues(__spreadValues({}, JSON.parse(core.getInput("secrets_string") || "{}")), getSecretEnvironmentVariables(process.env));
  ...
  const deployParams = {
    ...
    type,
    uri: deploymentUri,
    isReleaseChannel: isReleaseChannel != null ? isReleaseChannel : false,
    branch,
    ...
    deployerName,
    proxyBufferSize,
    proxyBodySize: proxyBodySize && proxyBodySize.length > 0 ? proxyBodySize : void 0,
    resources,
    corsSettings: {
      allowedOrigins
    },
    ...
    workloadLabels: {
      ...
      "api.24sevenoffice.com/module": module2,
      ...
      "app.kubernetes.io/part-of": module2
    }
  };
  console.log(JSON.stringify(deployParams));
  try {
    const location = yield deploy(token, __spreadProps(__spreadValues({}, deployParams), { secrets }));
    yield waitForDeploymentToComplete(location, token);
  } catch (error) {
    core.setFailed(error);
  }
});
function waitForDeploymentToComplete(location, token) {
...
function getResourceSettings(c) {
  return {
    requests: {
...
function getSecretEnvironmentVariables(env) {
  return Object.keys(env || {}).filter((x) => x.indexOf("TFSO_SECRET_") == 0).reduce((prev, cur) => {
    prev[cur.replace("TFSO_SECRET_", "")] = env[cur];
    return prev;
  }, {});
}
function getEnvironmentVariables(env) {
  return Object.keys(env || {}).filter((x) => x.startsWith("TFSO_") && !x.startsWith("TFSO_SECRET_")).reduce((prev, cur) => {
