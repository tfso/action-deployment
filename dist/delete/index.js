...
var deleteDeployment = (authToken, uri, resource, env, module2, serviceName) => __async(void 0, null, function* () {
  const deploymentId = `${env}:${module2}:${serviceName}`;
  const response = yield (0, import_node_fetch.default)(`${uri}/${resource}/${deploymentId}`, {
    method: "DELETE",
    headers: {
...
var run = () => __async(exports, null, function* () {
  ...
  try {
    console.log(`\u{1F480} Deleting service ${serviceName} in namespace ${env}-${module2}`);
    yield deleteDeployment(token, deploymentUri, httpResource, env, module2, serviceName);
    console.log("\u{1F389} Deletion successfull!");
  } catch (error) {
